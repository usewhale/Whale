...
func TestChatLines_FocusSummaryStylesFromState(t *testing.T) {
	...
	entries := []UIMessage{{
		Role: "tool_summary",
		Kind: KindToolSummary,
		Text: "Denied 1 shell command, Command failed: go test ./..., Outside workspace: ../src, Mode hint: switch to /agent, HTTP 403 Forbidden: https://example.test, Reading 1 file: internal/tui/focus_view.go, Searched for 1 pattern",
		FocusSummary: &FocusSummary{
			Parts: []FocusSummaryPart{
				...
				{Kind: "read", State: "blocked", Count: 1, Action: "Outside workspace", Detail: "../src"},
