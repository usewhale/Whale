...
func TestImmutableSystemPromptIncludesDelegationPolicyBeforeToolSpecs(t *testing.T) {
	...
	for _, want := range []string{"Use parallel_reason for 2-8 independent", "Use spawn_subagent for one bounded exploration", "Use a single agent for direct questions", "Do not load a skill first unless the user explicitly names one"} {
